...
## 🐳 Container Execution (Docker-First Development)

**ENFORCEMENT: All npm/node commands MUST run inside Docker containers.**

### BLOCKED Commands (Never Run on Host)
```
npm install, npm ci, npm run, npm test, npm exec
npx <anything>
yarn add, yarn install, yarn run
pnpm add, pnpm install, pnpm run
node <script>, tsx <script>, bun <script>
```

### REQUIRED: Use docker exec
```bash
# All Node.js commands MUST run in container: my-project-dev-1
...
**Note:** Use `docker exec` WITHOUT `-it` flag in Claude Code (avoids "not a TTY" errors).

### Pre-Flight Check (MANDATORY)
Before running ANY npm/node command, verify container is running:
```bash
docker ps --filter "name=my-project-dev-1" --format "table {{.Names}}\t{{.Status}}"
```
...
|-----------|-------|--------|
| `npm install` | Container | Packages stay in container |
| `npm test/build` | Container | Uses container environment |
| `git` commands | Host | Manages source files |
| File editing | Host | Volume mount syncs automatically |
...
└── Docker Container (my-project-dev-1)
    ...
    └── Dev server (port mapped to host)
...
**Key Point:** `node_modules` exists ONLY in the container. Host stays clean.
